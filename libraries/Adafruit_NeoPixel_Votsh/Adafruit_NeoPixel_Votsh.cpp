...
Adafruit_NeoPixel_Votsh::Adafruit_NeoPixel_Votsh(uint16_t n, uint8_t p, uint8_t t) : numLEDs(n), numBytes(n * 3), pin(p), pixels(NULL)
...
Adafruit_NeoPixel_Votsh::~Adafruit_NeoPixel_Votsh() {
  if(pixels) free(pixels);
  pinMode(pin, INPUT);
}

void Adafruit_NeoPixel_Votsh::begin(void) {
  pinMode(pin, OUTPUT);
  digitalWrite(pin, LOW);
}

void Adafruit_NeoPixel_Votsh::show(void) {
...
void Adafruit_NeoPixel_Votsh::setPin(uint8_t p) {
  pinMode(pin, INPUT);
  pin = p;
  pinMode(p, OUTPUT);
  digitalWrite(p, LOW);
...
void Adafruit_NeoPixel_Votsh::setPixelColor(
...
void Adafruit_NeoPixel_Votsh::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
uint32_t Adafruit_NeoPixel_Votsh::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t Adafruit_NeoPixel_Votsh::getPixelColor(uint16_t n) const {
...
uint8_t *Adafruit_NeoPixel_Votsh::getPixels(void) const {
  return pixels;
}

uint16_t Adafruit_NeoPixel_Votsh::numPixels(void) const {
  return numLEDs;
}
...
void Adafruit_NeoPixel_Votsh::setBrightness(uint8_t b) {
